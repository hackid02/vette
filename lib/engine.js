...
export async function runAudit({
  ...
  // was the wallet declared by the caller? site-derived addresses are
  // treated differently: a contract found in site content is a disclosure
  // miss, not an auditable wallet.
  const addressProvided = !!address;
  ...
  if (url) {
    ...
    if (site.ok) {
      ...
      if (!address && site.addresses && site.addresses.length) {
        ...
        audit.notes.push(`No wallet provided — Vette took the first address published on the site (${shortAddr(address)}). The verdict below applies to THAT address, not to the website itself.`);
      }
  ...
  if (address) {
    ...
    if (isContract) {
      // a contract explicitly provided by the caller may be a multisig/smart
      // wallet — a contract merely found in site content is a disclosure miss:
      // the agent published a contract, not a wallet.
      audit.derivedContract = !addressProvided;
      audit.findings.unshift({
        level: "warning",
        title: "Published wallet is a CONTRACT, not a standard EOA wallet",
        detail: audit.derivedContract
          ? "The only address on this agent's page has bytecode deployed — a token or protocol contract, not a wallet. Vette will not issue a wallet verdict from a contract the agent never declared as its wallet."
          : "The address has bytecode deployed. Most agent wallets are EOAs. If this is claimed as a trading wallet, verify what the contract actually is and who controls it.",
        evidence: { type: "wallet", value: address, label: "contract code" },
      });
      if (audit.derivedContract) {
        audit.notes.push(
          "No wallet was declared — the address Vette found on the site is a CONTRACT. A contract found in site copy is not an auditable wallet, so no wallet verdict is issued. This is the disclosure miss itself."
        );
      }
    }
