...
import {
  getBalance, getCode, getTxs, getTokenTransfers, getTokenTransfersRpc, getApprovalLogs,
  currentAllowance, getSmartContract, getTokenSymbol, fetchSite, fmtEth, shortAddr,
...
export async function runAudit({
  ...
  if (address) {
    ...
    [balance, code, txs, transfers, scan] = await Promise.all([
      ...
      grab("explorer_token_transfers", async () => {
        try {
          ...
          record(id, "explorer_token_transfers", { address }, { count: t.length, source: "blockscout" }, "ERC20/721/1155 transfer history");
          return t;
        } catch (e) {
          record(id, "explorer_token_transfers", { address }, { error: String(e.message || e).slice(0, 80) }, "Blockscout transfer history unreachable — falling back to RPC log scan");
          const r = await getTokenTransfersRpc(address);
          if (!r.complete) {
            transfersPartial = true;
            audit.notes.push("Transfer history came from a PARTIAL RPC log scan (some windows failed) — breach counts are a lower bound, not the full picture.");
          }
          record(id, "rpc_transfer_scan", { address }, { count: r.items.length, complete: r.complete }, r.complete ? "Transfer events via RPC log scan (Blockscout was unreachable)" : "PARTIAL RPC transfer scan — lower bound only");
          return r.items;
        }
      }),
    ...
    // Was the transfer history reachable? If not, a mandate deviation check
    // CANNOT run — and must never be reported as "no breaches found".
    audit.transfersUnavailable = transfers === null || transfersPartial === true;
    audit.deviationCheckIncomplete = audit.mandateExplicit && (transfers === null || transfersPartial === true);
    ...
    for (const l of open) {
      ...
      probed.push({
        ...
        spenderMeta: meta || {},
        tokenSymbol:
          (transfers || []).find((t) => t.tokenAddr?.toLowerCase() === (l.tokenAddr || "").toLowerCase())?.token ||
          sym ||
          null,
      });
    }
    ...
    audit.breachCount = dev.breaches ?? 0;
    audit.findings = [...safety, ...dev.findings];
    audit.breachLowerBound = audit.deviationCheckIncomplete;
    if (audit.deviationCheckIncomplete) {
      audit.findings.push({
        level: "warning",
        kind: "deviation-incomplete",
        title: "Mandate deviation check could not complete — token transfer history partially unreachable",
        detail: "The explorer was unreachable (rate limit or outage), so the whitelist/never-sell checks ran on a partial RPC log scan (~3 days). Any breach count shown is a lower bound. Vette will not call the wallet clean on missing data — this is not a pass, it is an incomplete check. Re-run the audit for the full picture.",
        evidence: { type: "wallet", value: address, label: "retry the check" },
      });
    }
    if (isContract) {
  ...
  if (brokenEvidence) {
    ...
    audit.score = null;
  } else if (substantive.some((f) => f.level === "danger" || f.level === "critical")) {
    // A live danger/critical finding floors the verdict at DANGEROUS regardless
    // of score — a wallet with a door open to an unverified or scam contract is
    // never COMPLIANT. (Caught by the live-fire drill: one danger finding left
    // score 82, which the threshold alone called COMPLIANT.)
...
function buildNarrative(a) {
  ...
  lines.push("Every verdict above is a receipt. Nothing here is an opinion without one.");
  return lines;
}
