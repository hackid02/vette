...
import { fmtEth, fmtUnits, MAX_UINT_BIG, shortAddr, ageDays } from "./providers";

export const LEVELS = { critical: 40, danger: 18, warning: 8, info: 2 };

// Hardcoded top-20 (by broad market consensus) used for "top-N" mandate claims.
...
export function parseMandate(claims) {
  ...
  if (wl) {
    const syms = wl[1].split(/[^A-Z0-9]+/).filter((s) => s.length >= 2 && s !== "AND");
...
export function safetyFindings({ approvals, txs, transfers, balance, address }) {
  const f = [];
  ...
  if (!approvals || approvals.length === 0) {
    f.push({
      ...
      detail: "Vette found no ERC20/ERC721 Approval events owned by this wallet in the recent-block scan. Zero approval surface is the safest state.",
  ...
  for (const a of open) {
    ...
    if (a.value > 0n) {
      f.push({
        ...
        detail: `Verified contract, capped allowance. Healthy hygiene. ${ageDays(a.ts) ? `Approved ${ageDays(a.ts)}d ago.` : ""}`,
        evidence: { type: "tx", value: a.tx, label: "approval tx" },
      });
    }
  }
...
export function scoreFindings(findings) {
  let s = 100;
  for (const f of findings || []) s -= LEVELS[f.level] || 4;
  return Math.max(0, Math.min(100, s));
}

export function verdictFrom(score, { addressFound, anyEvidence }) {
