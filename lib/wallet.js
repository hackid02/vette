...
// Read whichever account is CURRENTLY selected in the wallet — no prompt,
// no re-authorization. This is the reliable way to follow account switches
// inside wallets (OKX, etc.) that don't re-emit accountsChanged or re-prompt.
...
// Real disconnect, two layers:
//   1. the caller detaches its listeners BEFORE calling this (no auto-reconnect)
//   2. EIP-2255 revoke makes the wallet itself forget the site, so the next
//      connect shows the wallet's own account picker again — this is what lets
//      you switch to a different wallet/account inside OKX.
export async function disconnectWallet(provider) {
  ...
  try {
    ...
    // some wallets reject or ignore revoke — the listener detach above still
    // stops the auto-reconnect; the wallet may just skip the account picker
  }
}

export async function ensureBase(provider) {  const chainId = await provider.request({ method: "eth_chainId" }).catch(() => null);
