...
async function jgetRetry(url, { attempts = 2, timeout = 12000, delay = 800 } = {}) {
  let lastErr = null;
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      // 5xx and 404 are not transient — fall through to the RPC fallback now
      // instead of burning the user's time on retries.
      if (/HTTP 5\d\d|HTTP 404/.test(msg)) break;
      const limited = msg.includes("HTTP 429");
