...
export function runExam({ text = "", links = [], addresses = [], eoas = [], contracts = [], codeCheckFailed = false, title = "", url = "" }) {
  ...
  for (const q of EXAM_QUESTIONS) {
    if (q.id === "wallet") {
      ...
      if (eoas.length > 0) {
        ...
        evidence = eoas[0];
        declared += 1;
      } else if (codeCheckFailed && addresses.length > 0) {
        grade = "DECLARED";
        evidence = `${addresses[0]} (contract check failed — Base read unreachable)`;
        declared += 1;
      } else if (contracts.length > 0) {
        grade = "CONTRADICTED";
        evidence = `The only address published is a CONTRACT — code, not the wallet that signs.`;
        contradicted += 1;
      } else {
        grade = "ABSENT";
        evidence = null;
  ...
  const hasWallet = eoas.length > 0 || (codeCheckFailed && addresses.length > 0);
  ...
  const contractOnly = !hasWallet && contracts.length > 0;
  const note =
    verdict === "REFUSED TO SIT" && contractOnly
      ? "The only address published is a CONTRACT — code, not the wallet that signs. Publishing a token address is not answering the exam. Refused to sit by default."
      : verdict === "REFUSED TO SIT"
      ? "No operative wallet published. An agent that cannot be held to a wallet refuses the exam by default — the only honest grade."
