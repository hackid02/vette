...
// GET /api/card?v=VERDICT&s=SCORE&t=TARGET → shareable verdict card (PNG).
// Builders post this link on X/Discord to show their Vette score.
const COLORS = {
...
export async function GET(req) {
  ...
  const v = String(searchParams.get("v") || "UNVERIFIABLE").toUpperCase();
  const s = searchParams.get("s");
  const t = String(searchParams.get("t") || "").slice(0, 90);
  const c = COLORS[v] || COLORS.UNVERIFIABLE;
